...
pub fn new_title () -> (Entity, VecDeque<Component>) {
    ...
    text_box.transform.set_width(TextComponent::get_text_offset(&text_box.text, &text_box.font).get_x() as i32 * 4);
    ...
    ent.transform.nudge_y((text_box.transform.get_height() / 2) as f32);
...
pub fn new_to_misc() -> (Entity, VecDeque<Component>) {
    ...
    ent.transform.position.nudge_y( -40.0);
...
pub fn new_to_title() -> (Entity, VecDeque<Component>) {
    ...
    ent.transform.nudge_y( (screen().h() / 2 * 17 / 20) as f32 );
    ent.transform.nudge_x((screen().h() / 2 * 3 / 4) as f32 * -1.0);
